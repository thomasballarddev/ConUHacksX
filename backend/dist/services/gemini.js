...
import { initiateClinicCall, getActiveCallStatus } from './elevenlabs-call.js';
...
async function executeFunctionCall(name, args) {
    ...
    switch (name) {
        case "show_nearby_clinics": {
            const symptoms = args.symptoms || 'general health concerns';
        ...
        case "initiate_clinic_call": {
            const clinicName = args.clinic_name;
        ...
        case "trigger_emergency": {
            const reason = args.reason || 'Critical medical emergency detected';
...
export async function sendChatMessage(message, conversationId) {
    ...
    try {
        ...
        if (functionCalls && functionCalls.length > 0) {
            ...
            const functionResult = await executeFunctionCall(fc.name, fc.args);
 ...
 */
async function generateHealthProfile() {
