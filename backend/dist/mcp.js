...
mcpServer.tool("initiate_call", "Initiate an outbound phone call with a message", {
    ...
    try {
        ...
        const callSid = await initiateCall(params.phoneNumber, `You are calling ${params.recipientName || 'the recipient'} on behalf of the user. Your message: ${params.message}`, `Hello! This is Health.me calling. ${params.message}`, ngrokHost);
        return {
            content: [
                {
                    type: "text",
...
// Tool: End Call
mcpServer.tool("end_call", "End an active phone call", {
...
mcpServer.tool("get_call_status", "Get the status and transcript of an active call", {
...
mcpServer.tool("send_chat_message", "Send a text message to the ElevenLabs health assistant and get a response", {
